...
VALUES ("2018-05-09 10:10:30", "konstantin@mail.ru", "konstantin", "password", "img/konstantin.jpg", "Russia"),
       ("2018-05-09 11:15:20", "boris@mail.ru", "boris", "password", "img/boris.jpg", "Russia");
...
SELECT lots.name, lots.initial_price, lots.image, categories.name, (
  SELECT COUNT(bets.lot_id)
  FROM bets
  WHERE lots.id = bets.lot_id
  GROUP BY bets.lot_id
) count_bets, (
  SELECT MAX(bets.amount) + lots.initial_price
  FROM bets
  WHERE lots.id = bets.lot_id
) current_price FROM lots
