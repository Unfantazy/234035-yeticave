<nav class="nav">
    <ul class="nav__list container">
        <?php foreach ($inf as $i => $value_data): ?>
        <?php $class_name = ($extra['category'] === $value_data['name']) ? "nav__item--current" : "";?>
            <li class="nav__item <?=$class_name; ?>">
                <a href="all-lots.php?category=<?=$value_data['name']; ?>"><?=$value_data['name']; ?></a>
            </li>
        <?php endforeach; ?>
    </ul>
</nav>
<div class="container">
    <section class="lots">
        <h2>Все лоты в категории <span>«<?=$extra['category']; ?>»</span></h2>
        <ul class="lots__list">
            <?php if (empty($data)): ?>
